...
if __name__ == '__main__':
  ...
  #記録用フォルダの生成
  record_dir_path = '../analyzed_data/' + protocol + '/' + device + '/ProcTime/' + level_shift + '/'
  ...
  for speed_hz in speed_hz_arr:
    ...
    try:
      fetch_data_dir = '../measured_data/' + protocol + '/' + device + '/ProcTime/' + level_shift + '/'
      proc_file = str(speed_hz) + 'Hz_' + str(send_bytes) + 'bytes.txt'
      proc_file_path = fetch_data_dir + proc_file
      with open(proc_file_path, mode = 'r', encoding = 'utf-8') as fh:
