...
###########################################################
# それぞれのスループットのデータからbaudrateとレベルシフト方法による比較を行い、スループットとエラー率をそれぞれ棒グラフと折れ線グラフにまとめる。
...
#         throuput_graph.grid(which='minor',color='gray',linestyle='dotted')
    #   plt.yscale('log')
      ...
      for shift_way in ['FET', 'reg_div']:
        linecolor = "steelblue" if shift_way == 'FET' else 'orange'
...
#         speed_hz_arr = [10000, 14400, 19200, 28800, 38400, 57600, 100000, 115200]
        for speed_hz in speed_hz_arr:
          ...
          if val_found == False:
            ...
            print('Val not found. : ' + comm_way.upper() + '/ ' + str(speed_hz))
            continue
          # グラフ描画用に値を保存
        ...
        # グラフを描画
        # 断続的なグラフにを描画するために余計な処理が必要(何もしないと、断続的なグラフ全てにラベルがついて、凡例が膨大になったりする)
        throuput_arr_draw = []
        error_rate_arr_draw = []
        spdhz_arr_draw = []
        is_labeled = False
        ...
        for throuput, error_rate, spdhz in zip(throuput_array, error_rate_array, spdhz_array_for_draw):
          if throuput != '#':
            ...
            if len(throuput_arr_draw) > 0:
              if is_labeled == False:
                error_graph.plot([i/1000 for i in spdhz_arr_draw], error_rate_arr_draw, linewidth = 2, color=linecolor, label=shift_way)
                throuput_graph.plot([i/1000 for i in spdhz_arr_draw], throuput_arr_draw, linewidth = 2, color=linecolor, label=shift_way)
                is_labeled = True
              else:
                error_graph.plot([i/1000 for i in spdhz_arr_draw], error_rate_arr_draw, linewidth = 2, color=linecolor)
                throuput_graph.plot([i/1000 for i in spdhz_arr_draw], throuput_arr_draw, linewidth = 2, color=linecolor)
            throuput_arr_draw = []
            error_rate_arr_draw = []
            spdhz_arr_draw = []
            is_continuous = False
      ...
      folder_dir = comm_way.upper() + '/' + delay_way + comm_way.upper() + 'compare_levelshift_throuput_' + ack_way + '/'
      ...
      try:
        ...
        pass
      fig1.savefig(pdf_path + '/' + str(send_bytes) + 'bytes_compare_error_rate_by_baud.pdf')
      fig1.savefig(png_path + '/' + str(send_bytes) + 'bytes_compare_error_rate_by_baud.png')
      fig2.savefig(pdf_path + '/' + str(send_bytes) + 'bytes_compare_throuput_by_baud.pdf')
      fig2.savefig(png_path + '/' + str(send_bytes) + 'bytes_compare_throuput_by_buad.png')
      plt.close()

      print(comm_way.upper() + ': ' + ack_way + ' : Record image of ' + str(send_bytes))
      print(png_path)
