...
###########################################################
# 1byteの生データから、最大のスループットを出せる通信速度について、それぞれの通信方法による
...
for shift_way in ['FET']:
  ...
  for num, (comm_way, delay_way) in enumerate(zip(comm_way_arr, ['nodelay', '', ''])):
    if comm_way == 'SPI':
      speed_hz = 801282
    elif comm_way == 'i2c':
      speed_hz = 25270
  ...
  if output == 0:
    ...
    compare_graph.set_title('Compare 2Delay')
...
#     compare_graph.set_xlim(xmax =  9)
    compare_dir = 'compare_max_2delay(' + shift_way + ')/'
    ...
    plt.savefig(pdf_path + '/' + str(speed_hz) + 'Hz.pdf')
    plt.savefig(png_path + '/' + str(speed_hz) + 'Hz.png')
    plt.close()
...
#     compare_graph.set_xlim(xmax =  9)
    ...
    try:
      ...
      pass
    png_name = str(speed_hz) + 'Hz(stack_graph).pdf'
    plt.savefig(pdf_path + '/' + str(speed_hz) + 'Hz(stack_graph).pdf')
    plt.savefig(png_path + '/' + str(speed_hz) + 'Hz(stack_graph).png')
    plt.close()
    print('Recorded image')
