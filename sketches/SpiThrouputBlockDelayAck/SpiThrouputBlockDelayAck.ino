...
#define UPPER_READ_OR_ACK_SEND 0
#define LOWER_READ 1
#define DATA_READ 2
#define ACK_SEND 3

static int status = UPPER_READ_OR_ACK_SEND;
...
ISR (SPI_STC_vect) {
  switch(status){
    case UPPER_READ_OR_ACK_SEND:
      /* ACK_SENDとして呼ばれたときは下のコードは意味をなさない
       * DATA_READのときにSPDRに入れた値をRPiに読んでもらうため
       * だけにこのcaseが呼び出される
       */
      upper_lng = SPDR;
      status = LOWER_READ;
