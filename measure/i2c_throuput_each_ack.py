...
def getdata(send_bytes):
  ...
  err = 0
  for i, sendb in enumerate(send_bytes):
    # 送受信
    bus.write_i2c_block_data(SLAVE_ADDRESS, 0, [sendb])
    if i == (len(send_bytes)-1):
      result = bus.read_i2c_block_data(SLAVE_ADDRESS, 255, 1)
    else:
      result = bus.read_i2c_block_data(SLAVE_ADDRESS, 0, 1)
